...
# model io
class Salvatore_Checkpoint_Loader_Model_Name:
    @classmethod
    def INPUT_TYPES(s):
...
# sampling
class Salvatore_KSamplerAdvanced:
    @classmethod
    def INPUT_TYPES(s):
        return {"required":
                    {"model": ("MODEL",),
...
class Salvatore_Alternating_KSamplerAdvanced:
    @classmethod
    def INPUT_TYPES(s):
        return {"required":
                    {"model": ("MODEL",),
...
# utilities
class Salvatore_Seed_to_Number:
    def __init__(self):
        pass
    ...
    CATEGORY = "Salvatore Nodes/number"

    def number_to_seed(self, seed):
        return (int(seed["seed"]), )

class Salvatore_Seed_and_Int:
    def __init__(self):
        pass
...
# in the input_string
class Salvatore_Simple_Pattern_Replace:
    def __init__(self):
        pass
...
class Salvatore_SDXL_Resolutions:
...
class Salvatore_Resolutions_by_Ratio:
...
class Salvatore_Empty_Latent_Image_By_Resolution:
...
# latent
class Salvatore_Empty_Latent_Image_By_Ratio:
...
class Salvatore_Empty_Latent_Image_By_Pixels:
...
class Salvatore_SDXL_Quick_Empty_Latent:
    resolution = ["1024x1024|1:1","1152x896|9:7","1216x832|19:13","1344x768|7:4","1536x640|12:5"]
    direction = ["landscape","portrait"]
...
class Salvatore_SDXL_Resolution_Multiplier:
    def __init__(self, device="cpu"):
        self.device = device
...
# conditioning
class Salvatore_CLIP_Text_Positive_Negative:
    @classmethod
    def INPUT_TYPES(s):
...
class Salvatore_CLIP_Positive_Negative:
    @classmethod
    def INPUT_TYPES(s):
        return {"required": {
            "clip": ("CLIP", ),
...
class Salvatore_CLIP_Text_Positive_Negative_XL:
    @classmethod
    def INPUT_TYPES(s):
        return {"required": {
...
class Salvatore_CLIP_Positive_Negative_XL:
    @classmethod
    def INPUT_TYPES(s):
        return {"required": {
            "width": ("INT", {"default": 1024.0, "min": 0, "max": MAX_RESOLUTION}),
...
class Salvatore_CLIP_Text_Unified:
...
# upscaling
class Salvatore_Image_Scale_By_Factor:
...
class Salvatore_Image_Scale_By_Shortside:
    upscale_methods = ["nearest-exact", "bilinear", "area"]
...
class Salvatore_SDXL_Quick_Image_Scale:
    upscale_methods = ["nearest-exact", "bilinear", "area"]
    resolution = ["1024x1024|1:1","1152x896|9:7","1216x832|19:13","1344x768|7:4","1536x640|12:5"]
    direction = ["landscape","portrait"]
...
class Salvatore_Upscale_By_Factor_With_Model:
    upscale_methods = ["nearest-exact", "bilinear", "area"]
...
# outpainting
class Salvatore_Outpaint_To_Image:
...
class Salvatore_VAE_Encode_For_Inpaint_Padding:
    def __init__(self, device="cpu"):
        self.device = device
...
class Salvatore_Generate_Edge_Mask:
    directions = ["left","right","up","down"]
...
class Salvatore_Image_Save_With_Prompt:
    def __init__(self):
...
class Salvatore_Image_Save_With_Prompt_Info:
    def __init__(self):
        # get default output directory
        self.type = "output"
        self.output_dir = folder_paths.output_directory
...
class Salvatore_Image_Save_With_File_Info:
    def __init__(self):
        # get default output directory
        self.output_dir = folder_paths.output_directory
        self.type = "output"
...
class Salvatore_Image_Save_With_File:
    def __init__(self):
        # get default output directory
        self.output_dir = folder_paths.output_directory
        self.type = "output"
...
class Salvatore_Save_Prompt_File:
    def __init__(self):
        # get default output directory
        self.output_dir = folder_paths.output_directory
...
class Salvatore_Save_Prompt_File_Info:
    def __init__(self):
        # get default output directory
        self.output_dir = folder_paths.output_directory
...
class Salvatore_Save_Positive_Prompt_File:
    def __init__(self):
        # get default output directory
        self.output_dir = folder_paths.output_directory
...
class Salvatore_Image_Grayscale:
    @classmethod
    def INPUT_TYPES(s):
        return {"required": { "original": ("IMAGE",), }}
    RETURN_TYPES = ("IMAGE",)
...
class Salvatore_Read_Prompt:
    ...
    ''' Return order:
        ...
        width(int), height(int), positive_magic (string)
    '''
        ...
        if extension.lower() == 'png':
            try:
                ...
                if 'workflow' in metadata:
                    ...
                    if prompts:
                        # Join all extracted positive prompts
    ...
    def extract_positive_from_workflow(self, workflow_data, processed_nodes):
        """Extract positive prompts from workflow nodes - adapted from kprompt.py"""
        positive_prompts = []
...
class Salvatore_Build_Filename_String:
    def __init__(s):
        pass
...
NODE_CLASS_MAPPINGS = {
    #loaders
    "Checkpoint Loader w/Name (Salvatore)": Salvatore_Checkpoint_Loader_Model_Name,
    #samplers
    "KSamplerAdvanced (Salvatore)": Salvatore_KSamplerAdvanced,
    # "Alternating KSampler (Salvatore)": Salvatore_Alternating_KSamplerAdvanced,
    #conditioning
    "CLIP Positive-Negative (Salvatore)": Salvatore_CLIP_Positive_Negative,
    "CLIP Positive-Negative w/Text (Salvatore)": Salvatore_CLIP_Text_Positive_Negative,
    "CLIP Positive-Negative XL (Salvatore)": Salvatore_CLIP_Positive_Negative_XL,
    "CLIP Positive-Negative XL w/Text (Salvatore)": Salvatore_CLIP_Text_Positive_Negative_XL,
    "CLIP +/- w/Text Unified (Salvatore)": Salvatore_CLIP_Text_Unified,
    #latent
    "Empty Latent by Pixels (Salvatore)": Salvatore_Empty_Latent_Image_By_Pixels,
    "Empty Latent by Ratio (Salvatore)" : Salvatore_Empty_Latent_Image_By_Ratio,
    "Empty Latent by Size (Salvatore)": Salvatore_Empty_Latent_Image_By_Resolution,
    "SDXL Quick Empty Latent (Salvatore)" : Salvatore_SDXL_Quick_Empty_Latent,
    #image
    "Image Load with Metadata (Salvatore)": Salvatore_Read_Prompt,
    "Grayscale Image (Salvatore)": Salvatore_Image_Grayscale,
    #inpainting
    "Generate Border Mask (Salvatore)": Salvatore_Generate_Edge_Mask,
    "Outpaint to Image (Salvatore)": Salvatore_Outpaint_To_Image,
    "VAE Encode for Inpaint w/Padding (Salvatore)": Salvatore_VAE_Encode_For_Inpaint_Padding,
    #upscaling
    "Image Scale By Factor (Salvatore)": Salvatore_Image_Scale_By_Factor,
    "Image Scale by Shortside (Salvatore)": Salvatore_Image_Scale_By_Shortside,
    "SDXL Quick Image Scale (Salvatore)": Salvatore_SDXL_Quick_Image_Scale,
    "Upscale by Factor with Model (Salvatore)": Salvatore_Upscale_By_Factor_With_Model,
    #numbers
    "Multiply Integer (Salvatore)": Salvatore_Int_Multiply,
    "Quick Resolution Multiply (Salvatore)": Salvatore_Res_Multiply,
    "Resolutions by Ratio (Salvatore)": Salvatore_Resolutions_by_Ratio,
    "Seed to Number (Salvatore)": Salvatore_Seed_to_Number,
    "Seed and Int (Salvatore)": Salvatore_Seed_and_Int,
    "SDXL Steps (Salvatore)": Salvatore_SDXL_Steps,
    "SDXL Resolutions (Salvatore)": Salvatore_SDXL_Resolutions,
    #text
    "Build Filename String (Salvatore)": Salvatore_Build_Filename_String,
    "Time String (Salvatore)": Salvatore_Time_String,
    "Simple Pattern Replace (Salvatore)": Salvatore_Simple_Pattern_Replace,
    "Simple String Combine (Salvatore)": Salvatore_String_Append,
    "Prompt Weight (Salvatore)": Salvatore_Prompt_Weight,
    #IO
    "Image Save with Prompt (Salvatore)": Salvatore_Image_Save_With_Prompt,
    "Image Save with Prompt/Info (Salvatore)": Salvatore_Image_Save_With_Prompt_Info,
    "Image Save with Prompt File (Salvatore)": Salvatore_Image_Save_With_File,
    "Image Save with Prompt/Info File (Salvatore)": Salvatore_Image_Save_With_File_Info,
    "Save Prompt (Salvatore)": Salvatore_Save_Prompt_File,
    "Save Prompt/Info (Salvatore)": Salvatore_Save_Prompt_File_Info,
    "Save Positive Prompt(Salvatore)": Salvatore_Save_Positive_Prompt_File
}
